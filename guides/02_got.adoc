...
For this section it's best to start with a blank slate so run the following query to delete everything we've created so far:
...
In the next section we're going to look at the relationships between different characters.
